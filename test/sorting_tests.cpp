...
TEST_CASE("Sorting test sorted (ascending order) data",
  ...
  SECTION("Quick sort") {
    sri::quick_sort(actual.begin(), actual.end());
    REQUIRE_THAT(actual, Equals(sorted));
  }
  SECTION("Iterative quick sort") {
    sri::quick_sort_i(actual.begin(), actual.end());
    REQUIRE_THAT(actual, Equals(sorted));
  }
}
...
TEST_CASE("Sorting test sorted (descending order) data",
  ...
  SECTION("Quick sort") {
    sri::quick_sort(actual.begin(), actual.end(), std::greater<>());
    REQUIRE_THAT(actual, Equals(sorted));
  }
  SECTION("Iterative quick sort") {
    sri::quick_sort_i(actual.begin(), actual.end(), std::greater<>());
    REQUIRE_THAT(actual, Equals(sorted));
  }
}
...
TEST_CASE("Sorting test random data", "[sorting_tests.random_data]") {
  ...
  SECTION("Quick sort") {
    sri::quick_sort(unsorted.begin(), unsorted.end());
    REQUIRE_THAT(unsorted, Equals(sorted));
  }
  SECTION("Iterative quick sort") {
    sri::quick_sort(unsorted.begin(), unsorted.end());
...
TEST_CASE("Sorting test struct data", "[sorting_tests.struct_data]") {
  ...
  sri::RandomNumbers random;
  auto size = GENERATE(0, 1, 2, 5, 100);
  std::vector<sri::TestData> unsorted;
  unsorted.reserve(static_cast<std::vector<sri::TestData>::size_type>(size));
  for (auto i = 0; i < size; i++) {
    unsorted.emplace_back(random(), "hello");
  }
  std::vector<sri::TestData> sorted(unsorted);
  std::sort(sorted.begin(), sorted.end());
  ...
  SECTION("Iterative quick sort") {
    sri::quick_sort_i(unsorted.begin(), unsorted.end());
    REQUIRE_THAT(unsorted, Equals(sorted));
  }
}
